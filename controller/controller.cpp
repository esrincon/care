...
// Helper: Fetch and convert all scrub points from Redis into a vector of Vector3d
vector<Vector3d> getScrubPoints(RedisClient& redis_client, const string& key) {
    vector<Vector3d> points;
    vector<string> scrub_points_str = redis_client.getList(key);

    for (const auto& s : scrub_points_str) {
        istringstream ss(s);
        double x, y, z;
        ss >> x >> y >> z;
 ...
 int main() {
	if (simulation) {
	 ...
	 vector<Vector3d> scrub_points = getScrubPoints(redis_client, SCRUB_POINTS_KEY);
	 cout << "scrub points: " << scrub_points << endl;
		 ...
		 if (state == POSTURE) {
 		 ...
 		 // 
			 ...
			 if (scrub_points.size() > scrub_index) {
				double offset = 0.20;
				Vector3d ee_pos_desired = scrub_points[scrub_index];
			 ...
			 if ((ee_pos_current - ee_pos_desired).norm() < thresh) {
				...
				pose_task->setPosControlGains(0.0, 20.0, 0.0);
			 ...
			 const double torso_center_z    = 0.492;     // from <origin xyz=…>
			 const double torso_half_height = 0.3 / 2; // box size z = 0.5
